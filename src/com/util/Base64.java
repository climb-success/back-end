...
public class Base64
{
    ...
    /**
     * Converts a binary array to readable text using Base64 encoding.
     * The string can then be embedded in the database or text file.
     * @param input the byte array to be encode.
    ...
    public static String encode(byte[] input, int offset, int len)
    {
        ...
        for (src = offset; src < end-2; src += 3)
        {
            ...
            buf[dest++] = encode[(input[src+1] >>> 4) & 017 |
                                 (input[src] << 4) & 077];
            buf[dest++] = encode[(input[src+2] >>> 6) & 003 |
                                 (input[src+1] << 2) & 077];
            buf[dest++] = encode[input[src+2] & 077];
        }
     ...
     * This is the reverse of Base64.encode().
    ...
    public static byte[] decode(String input)
    {
        ...
        for (src = 0, dest=0; dest < buf.length-2; )
        {
            ...
            buf[dest++] = (byte) (((b1 << 2) & 255) |
                                  ((b2 >>> 4) & 003));
            buf[dest++] = (byte) (((b2 << 4) & 255) |
                                  ((b3 >>> 2) & 017));
            buf[dest++] = (byte) (((b3 << 6) & 255) |
                                   (b4 & 077));
        }
    ...
    /**
     * This is to convert a serializable object to readable text that can
     * then be embedded in the database or an XML file.
     * @param input the byte array to be encode.
     * @return an encoded string.
     * @since 1.0
     */
    ...
    /**
     * Decodes Base64-encoded string back to a seriazable object.
     * This is the reverse of Base64.encode().
     * @param input the Base64 encoded string to decode.
     * @return an decoded byte array.
     * @see Base64#encode(byte[])
